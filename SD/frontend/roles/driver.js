(function () {
  ...
  function deliveryDetails(item, local) {
    ...
    return `<button class="driver-back" data-driver-action="delivery-back" type="button">‹ My Deliveries</button><section class="driver-section driver-delivery-detail"><header><div><span class="foundation-eyebrow">Delivery request</span><h2>${esc(item.shipmentId)}</h2></div>${window.VitaeUI.badge(item.status)}</header><strong class="driver-detail-product">${esc(item.productCategory)}</strong><dl class="driver-detail-facts">${detail("Pickup location", item.pickup)}${detail("Destination", item.destination)}${detail("Pickup time", shortDate(item.departureAt))}${detail("Delivery deadline", shortDate(item.deadline))}${detail("Required temperature", range(item))}${detail("Container condition", `${temperature(item.temperature)} · ${human(item.coolingUnitStatus || item.sensorStatus || "unknown")}`)}${detail("Organization contact", item.organizationContact || "Not provided")}${detail("Special handling", item.specialHandlingInstructions)}</dl>${workflow}</section>`;
  }
