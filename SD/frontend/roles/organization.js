(function () {
  ...
  function shipmentRows(items, compact = false) {
    ...
    return `<div class="org-shipment-table ${compact ? "compact" : ""}"><div class="org-table-head"><span>Shipment</span><span>Destination</span><span>Latest accepted reading</span><span>Condition / lifecycle</span><span>Updated / ETA</span></div>${items.map((item) => `<button class="org-table-row" data-org-action="shipment-detail" data-id="${esc(item.shipmentId)}" type="button"><span><strong>${esc(item.shipmentId)}</strong><small>${esc(item.productName || item.productCategory)} · to ${esc(item.destinationHospitalName)}</small>${v2Lifecycle(item)}</span><span><strong>${esc(item.destinationHospitalName)}</strong><small>${esc(item.driverName || item.driverId || "Unassigned")}</small></span><span><strong>${temperature(item.temperature)}</strong><small>${item.batteryLevel == null ? range(item) : `${item.batteryLevel}% battery · ${range(item)}`}</small></span><span>${ui.badge(item.conditionStatus || item.riskLevel)} ${ui.badge(item.status)}</span><span><strong>${dateTime(item.lastUpdated || item.updatedAt)}</strong><small>ETA ${dateTime(item.expectedArrival)}</small></span></button>`).join("")}</div>`;
  }
